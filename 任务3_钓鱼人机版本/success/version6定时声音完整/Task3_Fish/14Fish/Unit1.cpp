...
Card  *cards[52] ;  //total cards     //数据结构使用队列 无用的卡放到最后，，这样就不在搜索了，每次从头开始找一定范围内的随即
Card  *myFish[3] ;
Card  *opponentFish[3];
Card  *poolFish[8];
...
void __fastcall TForm1::MenuStartClick(TObject *Sender)
{
        ...
        {
             FishMeat = Form6->num;
             this->StatusBar1->Panels->Items[2]->Text="当前鱼饵数目"+AnsiString(FishMeat);
        }
        delete Form6 ;
        //start timer
...
int   __fastcall TForm1::GetOneCard()
{
        ...
        AnsiString fileName = "music\\result\\win.mp3";
        int result = 1 ;
        if(count == 0)
        {
        	if(this->MyScoreInt > this->OpponnetScoreInt)
                {
                        fileName = "music\\result\\win.mp3";
		        result = 1 ;

                }
                else
                if(this->MyScoreInt == this->OpponnetScoreInt)
                {      fileName = "music\\result\\lose.mp3";
                       result = 2 ;
                }
                else
                {
                ...
                this->MediaPlayer3->FileName = fileName ;
                this->MediaPlayer3->Open();
	...
	int place = random(count) ;
        count-- ;
        Card *tempCard = cards[count];
        cards[count] = cards[place] ;
        cards[place] = tempCard ;
        this->LabelCardsCount->Caption = count  ;
	return   count ; ;
}

AnsiString __fastcall TForm1::makeCardText()
{
   AnsiString wholeCard = "" ;
    for(int i = 0 ;  i < 50 ; i++)
    {
       AnsiString cardLevel="" ;
       switch (cards[i]->cardLevel)
       {
         case 4: cardLevel="黑桃" ;
         break;
         case 3: cardLevel="红花";
       ...
       cardLevel +=AnsiString(cards[i]->cardNo)+"\t";
       wholeCard +=cardLevel ;
       if((i+1) % 4 ==0)
       {
                wholeCard +="\n";
...
void __fastcall TForm1::MyImage1Click(TObject *Sender)
{
        ...
        if(switchCard == true)
        {
         AnsiString fileName =        "music\\card\\"+AnsiString(myFish[0]->cardNo)+".mp3";
         this->MediaPlayer2->FileName=          fileName ;
         this->MediaPlayer2->Open() ;
...
void __fastcall TForm1::MyImage2Click(TObject *Sender)
{
	 if(MyImage2State == false )
         ...
         if(switchCard == true)
        {
          AnsiString fileName =        "music\\card\\"+AnsiString(myFish[1]->cardNo)+".mp3";
         this->MediaPlayer2->FileName=          fileName ;
         this->MediaPlayer2->Open() ;
         this->MediaPlayer2->Play();
         }
...
void __fastcall TForm1::MyImage3Click(TObject *Sender)
{
	 if(MyImage3State == false )
         ...
         if(switchCard == true)
        {
          AnsiString fileName =        "music\\card\\"+AnsiString(myFish[2]->cardNo)+".mp3";
         this->MediaPlayer2->FileName=          fileName ;
         this->MediaPlayer2->Open() ;
         this->MediaPlayer2->Play();
         }
...
void __fastcall TForm1::N1Click(TObject *Sender)
{
        ...
        this->StatusBar1->Panels->Items[0]->Text = "当前游戏点数"+AnsiString(WINSCORE) ;
        delete Form4 ;
}
//---------------------------------------------------------------------------
...
void __fastcall TForm1::N2Click(TObject *Sender)
{
        ...
        this->StatusBar1->Panels->Items[1]->Text = "当前难度" + level ;
        delete Form5 ;
}
//---------------------------------------------------------------------------
...
void __fastcall TForm1::Timer1Timer(TObject *Sender)
{
        ...
        AnsiString tips = "剩余时间\n"+AnsiString(GameTime);
        this->StatusBar1->Panels->Items[3]->Text= tips ;

        this->Image1->Canvas->TextOutA(0,0,tips);
