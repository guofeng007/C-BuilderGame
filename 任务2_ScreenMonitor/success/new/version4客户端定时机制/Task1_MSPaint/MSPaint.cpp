...
void __fastcall TForm1 ::AcceptFiles(TMessage& Msg)
{
    ...
    if(fileExt != NULL)
    {
        if(!fileExt.IsEmpty())
        {
                if(fileExt==".jpg"||fileExt==".bmp")
                {
                        ...
                        if(this->Image1->Picture->Bitmap->Modified == true)
                        {
                           
        	        int Choose = MessageDlg("图片已经修改，是否需要保存",mtConfirmation,TMsgDlgButtons()<<mbYes<<mbCancel,0);
                        if(Choose == mrYes)
                                {
 ...
 Screen->Cursors[crAlt]  = LoadCursorFromFile("cursors/alt.cur");
      ...
      int X, int Y)
{
        ...
        switch(ToolState)
        {
                case dtAlt:
                        ...
                        {
                        //refresh origin
                        int dtX = X-altCurrentX;
                        int dtY = Y-altCurrentY;
                        int altX1 =altX + dtX ;
                        int altY1 =altY + dtY ;



                         //copy  Graphics::TBitmap *TmpBitmap =new Graphics::TBitmap();
                        Graphics::TBitmap *TmpBitmap = new Graphics::TBitmap ;
                        TRect rect  ;
                        rect.left = altX1 ;
                        rect.top = altY1 ;
                        rect.right = altX1 + altR ;
                        rect.Bottom = altY1 + altR ;
                        TmpBitmap->Width = altR;
                        TmpBitmap->Height = altR;
                        //draw tips
                        /*
                        this->Image1->Canvas->Rectangle(rect);
                        */
                        this->Image3->Left = altX1 ;
                        this->Image3->Top = altY1 ;

                       TmpBitmap->Canvas->CopyRect(TRect(0,0,TmpBitmap->Width,TmpBitmap->Height),this->Image1->Canvas,rect);
                        //paste

                       this->Image1->Canvas->Draw(X,Y,TmpBitmap);//location ??
      ...
      TShiftState Shift)
{
        if(Key ==VK_MENU)
        {
                if(altSelectSource == false )
                {
                TPoint p1,p2 ;
                GetCursorPos(&p1);
                p2 = this->Image1->ScreenToClient(p1);
                altX = p2.x ;
                altY = p2.y ;
                this->Image3->Visible = true ;
                this->Image3->Left = altX ;
                this->Image3->Top = altY ;
                altSelectSource = true ;
        }
...
void __fastcall TForm1::SpeedButton17Click(TObject *Sender)
{
         EraseSelect();
        ToolState = dtAlt ;
        this->Image1->Cursor = TCursor(crAlt);      
}
//---------------------------------------------------------------------------

void __fastcall TForm1::MenuItemHelpThemeClick(TObject *Sender)
{
        ShowMessage("1.绘制图章alt功能时，，\na.首先点击按钮，，\nb然后alt选择源位置, \nc按下鼠标选择目标位置，\nd移动鼠标开始复制图片 \n2.支持鼠标拖拽，只能拖拽一个图片，格式为jpg,bmp!\n made by guofeng  ") ;
}
//---------------------------------------------------------------------------
